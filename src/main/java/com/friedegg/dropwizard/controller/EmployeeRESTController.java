package com.friedegg.dropwizard.controller;

import com.friedegg.dropwizard.dao.EmployeeDB;
    ...
    public Response createEmployee(Employee employee) throws URISyntaxException {
        ...
        if (violations.size() > 0) {
            ...
            for (ConstraintViolation<Employee> violation : violations) {
                violationMessages.add(violation.getPropertyPath().toString() + ": " + violation.getMessage());
            }
            return Response.status(Response.Status.BAD_REQUEST).entity(violationMessages).build();
        }
        if (e != null) {
